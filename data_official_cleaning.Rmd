...
gov_lp <- gov_lp %>%
  ...
  mutate(categoria = fn_typos(categoria))
...
```{r}
gov_pop <- gov_pop %>%
  select(COD_CENSO, POBTOT, contains("AGE"), URBANA, RURAL)
```
...
```{r}
gov_ge <- read_xls("/Users/ken/Documents/UC Davis Documents/Research/acdip_map_population_project/data_official/INE_CENSO_2002_LUGAR_POBLADO/GRUPO_ETNICO.xls")
glimpse(gov_ge)
```
...
gov_ge <- gov_ge %>%
  select(COD_CENSO,  # unique ID
         POBTOT,  # total population (differs from TOTAL)
...
gov_lp2 %>%
  mutate(pop_comparison= ifelse(POBTOT.x==POBTOT.y, T, F)) %>%
  filter(pop_comparison==F)
# it appears two rows are not equal; so lets keep both.

gov_lp2 %>%
  filter(is.na(POBTOT.y))
# great no NA values for POBTOT.y
```
...
```{r}
gov_lp2 <- gov_lp2 %>%
  mutate(PercentMaya = PEMAYA/POBTOT.y)
```
...
gov_lp2 %>%
  ...
  summarize(
    ...
    total_pop = sum(POBTOT.y, na.rm=T),
    percent_maya = sum(PEMAYA, na.rm=T)/sum(POBTOT.y, na.rm=T), 
    n=n(),
    popped = length(POBTOT.y[POBTOT.y>0]), # pt with pop data
    no_pop = length(POBTOT.y[POBTOT.y==0]),  # pt with no population data.
...
```{r}
# Inclusive: minus empty categories.
